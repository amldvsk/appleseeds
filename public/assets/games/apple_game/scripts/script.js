...
//constants
{
	...
	var frameTimerDelay = 1000 / 30;
	//remain at 30 fps. all velocity values should be multiplied by 30/(1000/frameTimerDelay) so that physics won't be ruined by framerate.
}
//arrays
{
	var cQuestions;
...
function setup() {//setup or restart
	...
	//run frameListener every 1000/[frames per second] millis.
	setInterval(frameListener, frameTimerDelay);
}
...
window.requestAnimFrame = (function() {//The magic word is requestAnimFrame(please);
	...
	function(callback) {
		window.setTimeout(callback, frameTimerDelay);
	};
})();
(function animloop() {//go home, watch anime, dream of a world with really big swords, wake up, feel disappointed, try jumping off the roof, fail, go home.
	...
	if (canvas != null && browser != -1) {//only use this in chrome, for some reason it makes IE run worse
		repaint(ctx, buffer);
	}
})();
	...
	if (!startGame && (!endGame || results.length != 0)) {
		...
		for (var i = 0; i < questions[pos].answers.length; i++) {
			...
			if (!curr.popped) {
				...
				drawInParallax(buffer, propeller[curr.animFrame], curr.x - 63, curr.y - 45, 0);
				buffer.font = "normal 18px Arial";
				...
				{
					showText(buffer, ans[j], curr.x + 75, curr.y + 100 + j * 18);
				}
			}
		}
		//draw results
		...
		//logo
		drawInParallax(buffer, logo, 0, 400, 0);
		//draw score
		buffer.font = "normal 48px Arial";
		drawStrokedText(buffer, score, 50, 525);
	...
	if (!startGame && (!endGame || results.length != 0)) {
		...
		if (falling)//only if the apples are already falling...
		{
			for (var i = 0; i < questions[pos].answers.length; i++) {//move answers
				questions[pos].answers[i].y += questions[pos].answers[i].yVel*dt/30;
				if (alreadyPopped)
					questions[pos].answers[i].yVel += dt / 30;
				//accelerate answers if an answer was popped.
				else if (Math.floor(animation) % 2 == 0)
					questions[pos].answers[i].animate();
				//animate propeller.
				if (questions[pos].answers[i].y < canvas.height && !questions[pos].answers[i].popped)
					outOfScreen = false;
			}
		...
		//key frame animation
		//change the animation variable
		animation += 30 / (1000 / frameTimerDelay);
		if (animation > 360)
			animation = 0;
		//move camera around
		camera.x = mx + (Math.cos(animation * (Math.PI / 180)) * 250);
		camera.y = my + (Math.abs(Math.sin(animation * (Math.PI / 180))) * 250);

	}
	if (canvas != null && browser == -1) {
		repaint(ctx, buffer);
	}
}
				...
				if ((mx > curr.x) && (mx < (curr.x + 150)) && (my > curr.y) && (my < (curr.y + 180))) {//and if the click occurred inside the hitbox of one of the apples
					...
					for (var j = 0; j < questions[pos].answers.length; j++) {
						if (!questions[pos].answers[j].popped)
							questions[pos].answers[j].yVel = 15 * 30 / (1000 / frameTimerDelay);
...
function Answer(content, correct) {
	...
	this.yVel = (-5 - (Math.random() * 3)) * 30 / (1000 / frameTimerDelay);
	this.img = getImg();
	this.popped = false;
...
function Question(options, q, time, points)//a single question
{
	...
	function setStandardPosition() {
		...
		for (var i = 0; i < amount; i++) {
			...
			tempAns[rnd[i]].x = 250 + i * (1000 / amount);
			tempAns[rnd[i]].xVel = 0;
			tempAns[rnd[i]].yVel = screenHeight * frameTimerDelay / (time * 1000);
